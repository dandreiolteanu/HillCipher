...
class Matrix {
    ...
    /// Returns whether the matrix is inversable or not
    public var isInversable: Bool {
    ...
    public func getUnsafeLine(_ i: Int) -> [Double] {
        return matrix[i - 1]
    }
    ...
    func decryptionInverse(with numberOfLetters: Int) throws -> Matrix {
        guard determinant() != 0 else {
            throw MatrixError.NoInverseAvailable
        }

        guard self.size.columns == 2 && self.size.lines == 2 else {
            throw MatrixError.MatrixShouldBe2x2
        }

        var adjMatrix = Matrix(n: 2, m: 2)
        adjMatrix.matrix[0][0] = matrix[1][1]
        adjMatrix.matrix[1][1] = matrix[0][0]
        ...
        for i in 0..<self.size.lines {
            for j in 0..<self.size.columns {
                if adjMatrix.matrix[i][j] < 0 {
                    adjMatrix.matrix[i][j] += Double(numberOfLetters)
                }
            }
        }

        adjMatrix = modulusDeterminant * adjMatrix

        for i in 0..<self.size.lines {
            for j in 0..<self.size.columns {
                let integerValue = Int(adjMatrix.matrix[i][j])
                adjMatrix.matrix[i][j] = Double(integerValue % numberOfLetters)
            }
        }
